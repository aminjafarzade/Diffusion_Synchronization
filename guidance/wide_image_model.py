...
class WideImageModel(BaseModel):
    ...
    def forward_mapping(self, z_t, **kwargs):
        ...
        xts = torch.tensor([]).to(self.device)
        views = self.mapper
        # print(self.num_views)
        # print(views)

        for view in views:
            crop = z_t[:, :, view[0]:view[1], view[2]:view[3]]
            # print(view)
            # print(crop.shape)
            if crop.numel() == 0:
                continue
            xts = torch.cat((xts, crop))
    ...
    def inverse_mapping(self, x_ts, **kwargs):
        ...
        # print(views)
        latent_sz = x_ts.shape[1]
        h, w = views[-1][1], views[-1][3]

        z_t = torch.zeros((1, latent_sz, h, w)).to(self.device)
        overlap_counter = torch.zeros((1, latent_sz, h, w), dtype=torch.int32).to(self.device)

        for i, view in enumerate(views):
            # print(x_ts[i].shape)
            z_t[:, :, view[0]:view[1], view[2]:view[3]] += x_ts[i]
            overlap_counter[:, :, view[0]:view[1], view[2]:view[3]] += 1

        z_t /= overlap_counter.clamp(min=1)
